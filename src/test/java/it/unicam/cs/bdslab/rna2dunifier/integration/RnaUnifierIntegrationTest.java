...
class RnaUnifierIntegrationTest {
        ...
        output.lines()
                ...
                .forEach(line -> {
                    ...
                    assertEquals(14, cols.length,
                            "Every row should have 14 columns: " + line);
                });
    }
    ...
    void processBarnabaCanonical() throws IOException, ParseException {
        ...
        assertFalse(result.startsWith("Index"),
                "The canonical BPSEQ should not start with header");
        // Ogni riga deve avere esattamente 3 colonne
        result.lines().filter(l -> !l.isBlank()).forEach(line -> {
            String[] cols = line.split("\t");
    ...
    void processToFileContentMatchesProcess(@TempDir Path tempDir) throws IOException, ParseException {
        ...
        String fromFile = new String(java.nio.file.Files.readAllBytes(output.toPath()),
                StandardCharsets.UTF_8);

        assertEquals(direct, fromFile,
                "The written content on the file should be the same of the direct output");
    }
    ...
    void extendedLongerThanCanonical() throws IOException, ParseException {
        ...
        assertTrue(extended.length() > canonical.length(),
