...
class BarnabaParserTest {
    ...
    private InputStream resource(String resourceName) {
        InputStream is = getClass()
            .getClassLoader()
            .getResourceAsStream("rna-output/barnaba/" + resourceName);
    ...
    void test1YMO_A_specificPairs() throws Exception {
        ...
        p = s
            .getPairs()
            .stream()
            .filter(pr -> (pr.getPos1() == 1 && pr.getPos2() == 31))
            .findFirst()
            .get();
    ...
    void test2K95_A() throws Exception {
        ...
        boolean hasTHS = s
            .getPairs()
            .stream()
            .anyMatch(p -> p.getType() == BondType.LEONTIS_WESTHOF_tHS);
        assertFalse(hasTHS);
    }
    ...
    void test2M8K_A() throws Exception {
        ...
        Pair p = s
            .getPairs()
            .stream()
            .filter(pr -> (pr.getPos1() == 12 && pr.getPos2() == 45))
            .findFirst()
            .get();
    ...
    void test4PLX_A() throws Exception {
        ...
        boolean hasTSS = s
            .getPairs()
            .stream()
            .anyMatch(p -> p.getType() == BondType.LEONTIS_WESTHOF_tSS);
        assertTrue(hasTSS);
    }
    ...
    void testNoUnexpectedUnknown(String resourceName) throws Exception {
        ...
        assertTrue(
            s
                .getPairs()
                .stream()
                .anyMatch(p -> p.getType() != BondType.UNKNOWN)
        );
    }
